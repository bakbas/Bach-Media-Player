...
Phase 5 finale — Theme Universe completed.
...
- New package `@bach/tailwind` (424 B brotli / 2 KB budget): preset
  ...
  entry under the `bach-*` namespace. Works for both Tailwind 3 and
  4. A unit test asserts every entry in `CSS_VARIABLE_TOKENS` is
